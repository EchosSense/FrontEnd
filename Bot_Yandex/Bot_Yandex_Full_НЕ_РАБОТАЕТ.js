...
function F1()
{
    ...
    let X1 = document.getElementsByName('text')[0];
    let timerId = setInterval(()=>{time_word(X1,timerId)},getRandom(500,1000));

    // Нажимаем кнопку Поиск
    if (X1.value == random_word)    // <<<============== ЕСЛИ ПОЛЕ ВВЕДЕНО ТО НАЖАТЬ НА КНОПКУ!! КАК??????????????????????????????
    {
...
function time_word(input_form,timerId)
{
